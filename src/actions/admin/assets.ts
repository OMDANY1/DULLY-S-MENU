...
export async function uploadProductImage(
  ...
  try {
    ...
    // 4. Upload to Supabase Storage using privileged admin client to bypass RLS safely on the server
    ...
    // 5. Update Database relationships using privileged client
    let dbResult;
    if (existingAsset) {
    ...
    // 6. Success! Now safely clean up the old storage object using privileged client if it existed
    if (existingAsset) {
...
export async function removeProductImage(
  ...
  try {
    ...
    // 3. Delete database relation first using privileged client
    ...
    // 4. Then remove from storage using privileged client (Audit and Log Warning on Failure)
