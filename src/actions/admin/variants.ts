...
export async function createVariant(formData: any): Promise<ActionResult<any>> {
  try {
    ...
    // Derived oz from size_label (e.g. "16 OZ" -> 16)
    const match = size_label.match(/^(\d+)\s*OZ$/i);
    const derivedOz = match ? parseInt(match[1], 10) : null;
...
export async function updateVariant(id: string, formData: any): Promise<ActionResult<any>> {
  try {
    ...
    const { size_label, price, calories, calorie_note, is_active } = parsed.data;
